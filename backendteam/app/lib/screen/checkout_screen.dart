...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget _buildPickUpSection() {
    const Color activeGreen = Color(0xFF8C9862);
    String timeDisplay = _pickupTime != null
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Location must be within the UNIJI Tower.',
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(8),
                      borderSide: const BorderSide(color: activeGreen),
                    ),
                  ),
                ),
              ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            earnedStamps > 0
                ? 'Will get $earnedStamps Stamp'
                : 'No Stamps (Min. Rp 50.000)',
            style: TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.w600,
              color: earnedStamps > 0 ? const Color(0xFF8C9862) : Colors.grey,
            ),
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_availableVouchers.isEmpty)
            ...
            Column(
              children: [
                _buildVoucherOptionCard(null, 'Do not use voucher', 0),
                const SizedBox(height: 10),
                ..._availableVouchers.map(
                  (voucher) => Padding(
                    padding: const EdgeInsets.only(bottom: 10),
                    child: _buildVoucherOptionCard(
                      voucher['id'],
                      '10% Discount Voucher',
                      num.tryParse(
                            voucher['discount_percent']?.toString() ?? '10',
                          ) ??
                          10,
                    ),
                  ),
          ...
          if (_discount > 0) ...[
            ...
            Text(
              'Saved ${_formatPrice(_discount)} with voucher',
              style: const TextStyle(
                color: activeGreen,
                fontSize: 12,
                fontWeight: FontWeight.bold,
  ...
  Widget _buildVoucherOptionCard(int? voucherId, String title, num percent) {
    const Color activeGreen = Color(0xFF8C9862);
    final isSelected = _selectedVoucherId == voucherId;
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? activeGreen.withValues(alpha: 0.10)
              : Colors.white,
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                voucherId == null ? title : '$title (${percent.toInt()}%)',
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: isSelected ? FontWeight.bold : FontWeight.w600,
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            label,
            style: TextStyle(fontSize: 12.0, fontWeight: weight, color: color),
          ),
          Text(
            value,
            style: TextStyle(fontSize: 12.0, fontWeight: weight, color: color),
                  ...
                  : () async {
                      ...
                      if (_fulfillmentType == 'pickup' && _pickupTime == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Please select a pick up time'),
                          ),
                        );
                        return;
                      }
                      if (_fulfillmentType == 'delivery' &&
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Please fill out the Floor and Room'),
                          ),
                        );
                        return;
                      }
                      ...
                      final result = await ApiService.createOrder(
                        ...
                        deliveryFloor: _floorController.text.isNotEmpty
                            ? _floorController.text
                            : null,
                        deliveryRoom: _roomController.text.isNotEmpty
                      ...
                      if (result != null && result['success'] == true) {
                        ...
                        if (_selectedPaymentMethod == 'app_qr') {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Order sent to kitchen! Please pay at the cashier.',
                              ),
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Order failed. Please check backend connection.',
                            ),
                          ),
                        );
                      }
